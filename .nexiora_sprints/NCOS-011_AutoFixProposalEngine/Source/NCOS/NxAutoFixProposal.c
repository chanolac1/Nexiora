...
static void nx_add_suggestion(NxAutoFixProposal* out,
                              const char* category,
                              const char* evidence,
                              const char* action,
                              int confidence)
{
    NxAutoFixSuggestion* s;
...
static void nx_generate_suggestions(const char* text, NxAutoFixProposal* out)
{
...
static int nx_write_report(const char* path, const char* proposal_id, const char* source_report, const NxAutoFixProposal* out)
{
    FILE* fp;
    int i;
    fp = fopen(path, "wb");
    ...
    for (i = 0; i < out->suggestion_count; ++i) {
        const NxAutoFixSuggestion* s = &out->suggestions[i];
        fprintf(fp, "### %d. %s\n\n", i + 1, s->category);
