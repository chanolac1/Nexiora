...
int main(void)
{
    FILE* fp;
    NxAutoFixProposal p;
    const char* root = "Build/ncos011_test_sandbox";
    const char* report = "Build/ncos011_test_sandbox/analysis.md";
    ...
    check(NxAutoFixProposal_CreateFromReport(root, "demo_fix", report, &p) == 1, "proposal should succeed");
    ...
    check(strstr(p.suggestions[0].action, "") != NULL, "suggestion should have action text");
    return failures ? 1 : 0;
}
