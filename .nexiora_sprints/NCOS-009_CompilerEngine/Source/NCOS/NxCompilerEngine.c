...
static int nx_join(char* dst, size_t dst_size, const char* a, const char* b)
{
    ...
    len = strlen(a);
    if (len > 0 && a[len - 1] != '/' && a[len - 1] != '\\') sep = NX_PATH_SEP;
                                    ...
                                    NxCompilerRunResult* out_result)
{
    ...
    if (rc == 0)
    {
        ...
        snprintf(out_result->summary, sizeof(out_result->summary), "Comando ejecutado correctamente. Log: %s", out_result->log_path);
    }
    else
    {
        out_result->status = NX_COMPILER_RUN_FAILED;
        snprintf(out_result->summary, sizeof(out_result->summary), "El comando fallo. Revisa el log: %s", out_result->log_path);
    }
