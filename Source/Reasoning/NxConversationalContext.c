...
static int nx_contains_ci(const char* text,const char* needle){size_t i,n;if(!text||!needle)return 0;n=strlen(needle);if(n==0U)return 0;for(i=0U;text[i];++i){size_t j=0U;while(j<n&&text[i+j]&&tolower((unsigned char)text[i+j])==tolower((unsigned char)needle[j]))++j;if(j==n)return 1;}return 0;}
static int nx_has_reference(const char* q){return nx_contains_ci(q,"¿y ")||nx_contains_ci(q,"y como")||nx_contains_ci(q,"y cómo")||nx_contains_ci(q,"se diferencia")||nx_contains_ci(q,"eso")||nx_contains_ci(q,"esta ")||nx_contains_ci(q,"este ");}
static int nx_build_resolved_question(const char* question,const char* subject,char* out,size_t cap){int w;if(nx_contains_ci(question,"se diferencia")&&nx_contains_ci(question,"semantica")){w=snprintf(out,cap,"Compara %s con memoria semantica. Pregunta original: %s",subject,question);}else{w=snprintf(out,cap,"%s Contexto activo: %s",question,subject);}return w>=0&&(size_t)w<cap;}
...
NxCcStatus NxConversationalContext_Ask(const char* p,const char* q,NxCcTurnResult* o){FILE* f;char e[NX_CC_MAX_PATH]={0},s[NX_CC_MAX_TEXT]={0},t[64]={0};unsigned int turns=0;NxGroundedReasoningStatus rs;if(!p||!q||!o||!*q)return NX_CC_INVALID_ARGUMENT;memset(o,0,sizeof(*o));f=fopen(p,"rb");if(!f)return NX_CC_IO_ERROR;if(!nx_read_field(f,"evidence",e,sizeof(e))||!nx_read_field(f,"subject",s,sizeof(s))){fclose(f);return NX_CC_IO_ERROR;}if(nx_read_field(f,"turns",t,sizeof(t)))sscanf(t,"%u",&turns);fclose(f);if(nx_has_reference(q)&&s[0]){if(!nx_build_resolved_question(q,s,o->resolved_question,sizeof(o->resolved_question)))return NX_CC_INVALID_ARGUMENT;}else if(!nx_copy(o->resolved_question,sizeof(o->resolved_question),q))return NX_CC_INVALID_ARGUMENT;nx_copy(o->active_subject,sizeof(o->active_subject),s);rs=NxGroundedReasoning_Ask(e,o->resolved_question,&o->grounded);if(rs!=NX_GR_OK&&rs!=NX_GR_INSUFFICIENT_EVIDENCE){o->status=NX_CC_REASONING_ERROR;return o->status;}o->turn_index=turns+1;o->status=NX_CC_OK;f=fopen(p,"ab");if(!f)return NX_CC_IO_ERROR;if(fprintf(f,"turn.%u.question=%s\nturn.%u.resolved=%s\nturn.%u.status=%s\nturn.%u.confidence=%u\nturns=%u\n",o->turn_index,q,o->turn_index,o->resolved_question,o->turn_index,NxGroundedReasoning_StatusName(rs),o->turn_index,o->grounded.confidence,o->turn_index)<0){fclose(f);return NX_CC_IO_ERROR;}if(fclose(f)!=0)return NX_CC_IO_ERROR;return NX_CC_OK;}
