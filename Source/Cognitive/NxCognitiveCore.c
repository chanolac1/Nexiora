    ...
    NxCognitiveAnswer* answer_out)
{
    ...
    while (fgets(line, sizeof(line), f) != 0)
    {
        ...
        if (nx_contains_word(text, normalized))
        {
            score += 1;
        }
        if (score > 0)
        ...
        if (score > matches[0].score)
        {
            ...
            nx_copy_string(matches[0].text, sizeof(matches[0].text), text);
        }
        else if (score > matches[1].score)
        {
            matches[2] = matches[1];
            matches[1].score = score;
            nx_copy_string(matches[1].text, sizeof(matches[1].text), text);
        }
        else if (score > matches[2].score)
        {
            matches[2].score = score;
            nx_copy_string(matches[2].text, sizeof(matches[2].text), text);
        }
    }
    fclose(f);
    ...
    answer_out->matched_chunks = matched;
    answer_out->confidence = best_score >= 6 ? 78 : (best_score >= 3 ? 58 : 25);

    if (best_score <= 0)
    {
        (void)snprintf(answer_out->answer, sizeof(answer_out->answer),
            "No tengo suficiente evidencia local para responder con precision sobre '%s'. Ingiere mas archivos, transcripciones o documentos relacionados y vuelve a preguntar.",
            topic);
    }
    else
    {
        (void)snprintf(answer_out->answer, sizeof(answer_out->answer),
            "Con base en la memoria local sobre %s:\n\n1) %s\n2) %s\n3) %s\nConfianza: %d %%\nFuente: Knowledge/Cognitive/Topics/%s/chunks.txt",
            topic,
            matches[0].text[0] == '\0' ? "No encontre un fragmento principal." : matches[0].text,
            matches[1].text[0] == '\0' ? "No encontre un segundo fragmento relevante." : matches[1].text,
            matches[2].text[0] == '\0' ? "No encontre un tercer fragmento relevante." : matches[2].text,
