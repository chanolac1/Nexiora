...
{
    if (plan == NULL || found == NULL || found_size == 0U) { return 0; }
...
#if defined(_WIN32)
    {
        ...
        char prefix[160];
        int written = snprintf(prefix, sizeof(prefix), "%s*.vtt", plan->source_id);
        if (written < 0 || (size_t)written >= sizeof(prefix) || !nx_join(pattern, sizeof(pattern), plan->workspace, prefix)) { return 0; }
        handle = FindFirstFileA(pattern, &data);
        if (handle == INVALID_HANDLE_VALUE) { return 0; }
        do {
            if ((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0U) {
                const int ok = nx_join(found, found_size, plan->workspace, data.cFileName);
                FindClose(handle);
                return ok;
            }
...
#else
    {
        ...
        struct dirent* entry;
        const size_t id_length = strlen(plan->source_id);
        if (directory == NULL) { return 0; }
        while ((entry = readdir(directory)) != NULL) {
            const size_t name_length = strlen(entry->d_name);
            if (name_length > 4U && strncmp(entry->d_name, plan->source_id, id_length) == 0 && strcmp(entry->d_name + name_length - 4U, ".vtt") == 0) {
                const int ok = nx_join(found, found_size, plan->workspace, entry->d_name);
                closedir(directory);
                return ok;
            }
        }
