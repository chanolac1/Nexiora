# TESTING STANDARD

- Every capability includes focused unit and integration tests.
- Full release certification materializes all registered test executables before CTest.
- A missing executable is a build failure, never a skipped success.
- Release certification requires zero failed and zero not-run tests.
- Tests must be hermetic and repeatable.

## Pruebas de diálogo unificado
Toda fachada cognitiva debe probar respuesta fundamentada, contradicción preservada y apertura de brecha ante evidencia insuficiente. La CLI debe ser artefacto obligatorio de Pure Apply.

## Goal Graph QA Gate
Debe probar creación, persistencia, jerarquía, dependencias, orden determinista, transiciones inválidas, ciclo, recarga y artefactos CLI. La certificación debe ejecutar la suite completa dos veces y generar `certification-report.nxcert`.
