...
int main(void)
{
    ...
    if (!require_true(strstr(plan.download_command, "youtube:player_client=default") != NULL,
                      "subtitle-only client policy missing")) { return 1; }
    if (!require_true(strstr(plan.download_command, "-f best") != NULL,
                      "single-format policy missing")) { return 1; }
