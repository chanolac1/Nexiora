...
int main(void)
{
    ...
    if (!require_true(strstr(plan.download_command, "--js-runtimes") == NULL, "missing optional runtime must not block the plan")) { return 1; }
    if (!require_true(strstr(plan.download_command, "deno.exe") == NULL, "missing optional Deno path leaked into the plan")) { return 1; }
