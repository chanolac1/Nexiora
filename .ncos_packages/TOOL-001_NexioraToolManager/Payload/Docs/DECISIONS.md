...
## ADR-0005 — Aprendizaje web basado en evidencia textual verificable

**Estado:** Aceptada

Nexiora no declarará aprendizaje directo de un video web sin evidencia extraíble. Para YouTube, la primera implementación adquiere subtítulos o transcripciones mediante `yt-dlp`, conserva la URL de origen, normaliza el contenido y lo entrega al motor cognitivo. La ausencia de subtítulos produce un fallo explícito y nunca una respuesta inventada.

## ADR-TOOL-001 — Herramientas aisladas dentro de Nexiora
Las dependencias ejecutables no se instalan globalmente ni se asumen presentes en PATH. Nexiora las administra en `Tools/Managed`, con proveedor HTTPS permitido, instalación atómica, registro de hash y capacidad de reversión.
