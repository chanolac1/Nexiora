...
int main(void)
{
...

    {
        ...
        memset(&tx,0,sizeof(tx));
        expect(NxPackageManager_Rollback(root,"NCOS Transaction Test",&tx)==1,"manual rollback should succeed after no-op install");
        expect(tx.files_rolled_back>=1,"rollback should restore at least one file after no-op install");
    }
