...
NxDecoderStatus NxDecoder_CreatePlan(const NxDecoderTools* tools, const char* source_path,
    ...
    if (plan_out == NULL) return NX_DA_INVALID_ARGUMENT;
    memset(plan_out, 0, sizeof(*plan_out));
    if (tools == NULL || !nx_da_safe_argument(source_path)) {
        nx_da_copy(plan_out->message, sizeof(plan_out->message), "Invalid or unsafe source path.");
        return NX_DA_INVALID_ARGUMENT;
    }
    nx_da_copy(plan_out->source_path, sizeof(plan_out->source_path), source_path);
    if (!nx_da_file_exists(source_path)) {
        nx_da_copy(plan_out->message, sizeof(plan_out->message), "Source file does not exist.");
        return NX_DA_NOT_FOUND;
    }
    ...
    if (nx_da_equal_ci(extension, ".png") || nx_da_equal_ci(extension, ".jpg") || nx_da_equal_ci(extension, ".jpeg") || nx_da_equal_ci(extension, ".bmp") || nx_da_equal_ci(extension, ".tif") || nx_da_equal_ci(extension, ".tiff")) {
        if (!tools->tesseract_available) { nx_da_copy(plan_out->message, sizeof(plan_out->message), "Tesseract is not available in PATH."); return NX_DA_TOOL_UNAVAILABLE; }
        ...
        if (!tools->whisper_available) { nx_da_copy(plan_out->message, sizeof(plan_out->message), "Whisper CLI is not available in PATH."); return NX_DA_TOOL_UNAVAILABLE; }
        if (!nx_da_safe_argument(whisper_model_path)) { nx_da_copy(plan_out->message, sizeof(plan_out->message), "Whisper model path is invalid or unsafe."); return NX_DA_INVALID_ARGUMENT; }
        ...
        if (!tools->ffmpeg_available) { nx_da_copy(plan_out->message, sizeof(plan_out->message), "FFmpeg is not available in PATH."); return NX_DA_TOOL_UNAVAILABLE; }
        if (!tools->whisper_available) { nx_da_copy(plan_out->message, sizeof(plan_out->message), "Whisper CLI is not available in PATH."); return NX_DA_TOOL_UNAVAILABLE; }
        if (!nx_da_safe_argument(whisper_model_path)) { nx_da_copy(plan_out->message, sizeof(plan_out->message), "Whisper model path is invalid or unsafe."); return NX_DA_INVALID_ARGUMENT; }
        ...
        nx_da_copy(plan_out->message, sizeof(plan_out->message), "Source format is not supported by decoder adapters.");
        return NX_DA_UNSUPPORTED;
    }
