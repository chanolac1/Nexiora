...
{
    const char* base_root = root == NULL ? "." : root;
    if (base_root[0] != '\0' && strcmp(base_root, ".") != 0) {
        (void)NX_MKDIR(base_root);
    }
    char knowledge[NX_PLAN_MAX_PATH];
    char ncos[NX_PLAN_MAX_PATH];
    nx_join(knowledge, sizeof(knowledge), base_root, "Knowledge");
...
int NxPlanning_Status(const char* root, NxPlanStatus* out)
{
    ...
    while (fgets(line, sizeof(line), f) != NULL) {
        ...
        } else if (strstr(line, "\"type\":\"step_update\"") != NULL) {
            int idx = nx_parse_json_int(line, "index");
            char status_value[32];
            if (nx_parse_json_string(line, "status", status_value, sizeof(status_value))) {
                for (int i = 0; i < out->step_count; ++i) {
                    if (out->steps[i].index == idx) {
                        nx_copy(out->steps[i].status, sizeof(out->steps[i].status), status_value);
                        break;
                    }
                }
            }
    ...
    for (int i = 0; i < out->step_count; ++i) {
        if (strcmp(out->steps[i].status, "done") == 0) out->completed_count++;
    }
    fclose(f);
    return 1;
}
