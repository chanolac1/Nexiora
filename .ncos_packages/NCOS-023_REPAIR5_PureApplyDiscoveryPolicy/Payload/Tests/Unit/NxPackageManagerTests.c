...
int main(void)
{
...

    {
        ...
        write_text(pending_manifest,
                   ...
                   "version=1.0.0\n"
                   "apply.configure_preset=windows-msvc-release\n"
                   "apply.build_preset=release\n"
                   "apply.test_target=nexiora_test_suite_staged\n"
                   "apply.test_preset=release-tests\n"
                   "file=Payload/pending.txt=>Knowledge/NCOS/pending.txt\n");
        write_text(pending_payload, "pending package\n");
        {
            char legacy[384];
            char legacy_manifest[448];
            char legacy_payload_dir[448];
            char legacy_payload[512];
            written = snprintf(legacy, sizeof(legacy), "%s/DOCS-001_LEGACY", package_root);
            expect(written > 0 && (size_t)written < sizeof(legacy), "legacy package path should fit");
            written = snprintf(legacy_manifest, sizeof(legacy_manifest), "%s/manifest.npkg", legacy);
            expect(written > 0 && (size_t)written < sizeof(legacy_manifest), "legacy manifest path should fit");
            written = snprintf(legacy_payload_dir, sizeof(legacy_payload_dir), "%s/Payload", legacy);
            expect(written > 0 && (size_t)written < sizeof(legacy_payload_dir), "legacy payload directory should fit");
            written = snprintf(legacy_payload, sizeof(legacy_payload), "%s/legacy.txt", legacy_payload_dir);
            expect(written > 0 && (size_t)written < sizeof(legacy_payload), "legacy payload path should fit");
            make_dir(legacy);
            make_dir(legacy_payload_dir);
            write_text(legacy_manifest,
                       "id=DOCS-001 Legacy Package\n"
                       "version=9.9.9\n"
                       "file=Payload/legacy.txt=>Knowledge/NCOS/legacy.txt\n");
            write_text(legacy_payload, "legacy package\n");
        }
