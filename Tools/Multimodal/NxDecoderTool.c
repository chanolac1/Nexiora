...
int main(int argc, char** argv) {
    ...
    if ((strcmp(argv[1], "plan") == 0 || strcmp(argv[1], "execute") == 0) && argc >= 3) {
        ...
        printf("status=%s\nsource=%s\noperation=%s\noutput=%s\nprimary=%s\nsecondary=%s\nmessage=%s\n", NxDecoder_StatusToString(status), plan.source_path, NxDecoder_OperationToString(plan.operation), plan.output_text_path, plan.command_primary, plan.command_secondary, plan.message);
        if (status != NX_DA_OK) return 1;
        if (strcmp(argv[1], "plan") == 0) {
            printf("action=simulation-only; use execute to generate the text file.\n");
            return 0;
        }
