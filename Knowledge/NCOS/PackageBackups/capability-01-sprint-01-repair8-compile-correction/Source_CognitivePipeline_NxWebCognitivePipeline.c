...
NxWcpStatus NxWcp_BuildPlan(const char* root, const char* url, const char* language, NxWcpPlan* plan)
{
    ...
    nx_progress(10U, "Acquisition plan ready", 0UL);
    {
        ...
        if (nx_exists(managed_deno)) {
            written = snprintf(plan->download_command, sizeof(plan->download_command),
                "\"%s\" --skip-download --ignore-no-formats-error --write-auto-subs --write-subs --sub-langs \"%s\" --sub-format vtt --no-playlist --extractor-args "youtube:player_client=default" -f best --js-runtimes \"deno:%s\" --remote-components ejs:github --sleep-requests 2 --retries 5 --extractor-retries 3 --retry-sleep 10 -o \"%s\" \"%s\"",
                managed_tool, language, managed_deno, output_template, url);
        } else {
            written = snprintf(plan->download_command, sizeof(plan->download_command),
                "\"%s\" --skip-download --ignore-no-formats-error --write-auto-subs --write-subs --sub-langs \"%s\" --sub-format vtt --no-playlist --extractor-args "youtube:player_client=default" -f best --sleep-requests 2 --retries 5 --extractor-retries 3 --retry-sleep 10 -o \"%s\" \"%s\"",
...
NxWcpStatus NxWcp_ConvertWebVtt(const char* transcript_path, const char* source_url, const char* title, const char* knowledge_path)
{
    ...
    if (evidence_lines > 0U && fputs("content_end\n", output) == EOF) {
        fclose(input);
        fclose(output);
        remove(knowledge_path);
        return NX_WCP_IO_ERROR;
    }
...
static NxWcpStatus nx_map_mmci_status(NxMmciStatus status)
{
    switch (status) {
        case NX_MMCI_OK: return NX_WCP_OK;
        case NX_MMCI_INVALID_ARGUMENT: return NX_WCP_INVALID_ARGUMENT;
        case NX_MMCI_NOT_FOUND: return NX_WCP_SOURCE_NOT_FOUND;
        case NX_MMCI_FORMAT_ERROR:
        case NX_MMCI_NO_EVIDENCE: return NX_WCP_FORMAT_ERROR;
        case NX_MMCI_IO_ERROR: return NX_WCP_IO_ERROR;
        case NX_MMCI_OUTPUT_TOO_SMALL: return NX_WCP_OUTPUT_TOO_SMALL;
        default: return NX_WCP_COMMAND_FAILED;
    }
}

static int nx_analysis_id_from_dir(const char* analysis_dir, char* analysis_id, size_t analysis_id_size)
{
    const char* slash;
    const char* backslash;
    const char* start;
    if (analysis_dir == NULL || analysis_id == NULL || analysis_id_size == 0U) { return 0; }
    slash = strrchr(analysis_dir, '/');
    backslash = strrchr(analysis_dir, '\\');
    start = analysis_dir;
    if (slash != NULL && slash + 1 > start) { start = slash + 1; }
    if (backslash != NULL && backslash + 1 > start) { start = backslash + 1; }
    return nx_copy(analysis_id, analysis_id_size, start);
}
                                      ...
                                      size_t message_size)
{
    ...
    NxMmciStatus mmci_status;
    NxWcpStatus mapped;
    if (root == NULL || knowledge_path == NULL || domain == NULL ||
        analysis_id == NULL || analysis_id_size == 0U || message == NULL || message_size == 0U) {
        return NX_WCP_INVALID_ARGUMENT;
    }
    analysis_id[0] = '\0';
    message[0] = '\0';
    mmci_status = NxMmci_Analyze(root, knowledge_path, domain, &analysis);
    mapped = nx_map_mmci_status(mmci_status);
    if (mapped != NX_WCP_OK) {
        if (!nx_copy(message, message_size, analysis.message[0] == '\0' ?
                     "Cognitive integration failed without diagnostic detail." : analysis.message)) {
            return NX_WCP_OUTPUT_TOO_SMALL;
        }
        return mapped;
    }
    if (!nx_analysis_id_from_dir(analysis.analysis_dir, analysis_id, analysis_id_size) ||
        !nx_copy(message, message_size, "Evidence segmented and integrated directly into the cognitive registry and graph.")) {
        return NX_WCP_OUTPUT_TOO_SMALL;
    }
    return NX_WCP_OK;
}
...
NxWcpStatus NxWcp_Execute(const char* root, const char* url, const char* language, const char* domain, NxWcpResult* result)
{
    ...
    }
    {
        ...
        js_runtime[0] = '\0';
        if (nx_exists(managed_deno)) {
            const int runtime_written = snprintf(js_runtime, sizeof(js_runtime), "deno:%s", managed_deno);
            if (runtime_written < 0 || (size_t)runtime_written >= sizeof(js_runtime)) {
                result->status = NX_WCP_OUTPUT_TOO_SMALL;
                return result->status;
            }
        }
        ...
        arguments[argument_count++] = "--extractor-args";
        arguments[argument_count++] = "youtube:player_client=default";
        arguments[argument_count++] = "-f";
        arguments[argument_count++] = "best";
        if (js_runtime[0] != '\0') {
    ...
    nx_progress(80U, "Integrating evidence directly into the cognitive engine", 0UL);
    status = NxWcp_IntegrateKnowledge(absolute_root,
                                      plan.knowledge_path,
                                      domain == NULL ? "web" : domain,
                                      result->analysis_id,
                                      sizeof(result->analysis_id),
                                      result->message,
    ...
    nx_copy(result->message, sizeof(result->message), "YouTube transcript acquired, normalized and integrated directly into cognitive evidence.");
