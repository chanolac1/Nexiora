...
int main(void) {
    ...
    status = NxDecoder_CreatePlan(&tools, "nx_decoder_missing.png", "spa", "model.bin", &plan);
    if (!check(status == NX_DA_NOT_FOUND, "missing source was not reported")) return 1;
    if (!check(plan.output_text_path[0] == '\0', "failed plan exposed uninitialized output")) return 1;
    if (!check(strcmp(plan.source_path, "nx_decoder_missing.png") == 0, "failed plan did not preserve source")) return 1;
    if (!check(strstr(plan.message, "does not exist") != NULL, "missing source diagnostic absent")) return 1;
